...
**Word is the master.** `resume.pdf` in the repo root is Will's own export from
`resume-source/Will Coggins CV (restyled).docx` (gitignored — see
`resume-source/EDITING-NOTES.md`). It is *not* generated from this site.
...
4. Update the `<main>` block in `index.html` to match — **the page and the PDF
   will drift apart otherwise**, and that's the failure mode to watch for. There
   is no automation keeping them in sync.
5. Check the PDF's **File → Info → Properties → Author** says "William Coggins".
   A doc generated by a script can carry a tool name here.

The download is served as `WillCoggins 2026 CV.pdf` via the `download` attribute
on the two links in `index.html` — the file on disk stays `resume.pdf`.

`Ctrl+P` on the live page produces its own clean two-page CV from the print
stylesheet (nav hidden, no job split across a page break, link URLs printed).
That's a separate artifact from `resume.pdf` — useful as a sanity check that the
page still reads well on paper, but it is not what visitors download.
